...
public class LightningSkill implements CommandExecutor {
    ...
    public void blockPlaceTwo(Location loc) {
        ...
        for (double t = 0, radius = 10; t <= 360 && radius >= 0; t += 1) {
            ...
            double x = (Math.cos(r) * radius + location.getX());
            double z = (Math.sin(r) * radius + location.getZ());
            Block block = Bukkit.getWorld("world").getBlockAt((int) x, (int)location.getBlockY(), (int)z);
